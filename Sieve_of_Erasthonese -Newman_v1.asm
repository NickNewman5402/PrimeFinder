...
.data
list: .space 1024	#minimum required array size in Bytes

.text
li $t1, 1		#initialize p
li $a1, 1024		#set to the max number stored in the array
li $a2, 1024		#set to the minimum number of times the eraser loop needs to iterate (i.e. max value of p)
addi $a3, $a1, 1025	#set N to the max number + 1
...
addi $s1, $s1, 4 #<4>	#step to next array cell							
...
sub $t2, $a1, $t1	# <x>	#set loop limit according to p
mul $t3, $t1, 4			#t3=p*4 for address
add $s1, $s1, 0x1000	#<y>		#start eraser loop at the proper index


Composite_Eraser:
beq $t0, $s1, Loop_Initializer	# <z> #move to next p if dynamic limit has been reached
add $s1, $s1, -4 #<y>		#skip to next index 
sw $zero, 0($s1)	#<w>		#store 0 in current index (i.e. erase value in current index)
